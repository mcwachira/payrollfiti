...
describe('Kenya payroll rules', () => {
  ...
  it('applies personal relief so low earners owe zero PAYE', () => {
    const result = runPayrollCalculation(input({ earnings: { basicSalary: 3_000 } }), kenya);
  ...
  it('handles zero salary without error, still applying NHIF\'s flat minimum band', () => {
    const result = runPayrollCalculation(input({ earnings: { basicSalary: 0 } }), kenya);
    expect(result.grossPay).toBe(0);
    expect(result.tax.netTax).toBe(0);
    expect(result.statutoryDeductions).toEqual(
      expect.arrayContaining([expect.objectContaining({ code: 'NHIF', employeeAmount: 150 })]),
    );
  ...
  it('flags negative salary as a validation error but still computes a result', () => {
    const result = runPayrollCalculation(input({ earnings: { basicSalary: -1_000 } }), kenya);
    expect(result.validation).toEqual(
      expect.arrayContaining([expect.objectContaining({ field: 'basicSalary', severity: 'error' })]),
    );
  });
  ...
  it('prorates basic salary for a mid-period joiner', () => {
    ...
    const prorated = runPayrollCalculation(
      input({ period: { periodStart: '2026-07-01', periodEnd: '2026-07-31', employmentStartDate: '2026-07-16' } }),
      kenya,
    );
    ...
    expect(prorated.earnings.basicSalary).toBeCloseTo(full.earnings.basicSalary * prorated.prorationFactor, 2);
  ...
  it('does not prorate overtime, commission or bonus — those are already actuals', () => {
    const result = runPayrollCalculation(
      input({
        earnings: { basicSalary: 50_000, overtimeAmount: 1_000, bonusAmount: 2_000 },
        period: { periodStart: '2026-07-01', periodEnd: '2026-07-31', employmentStartDate: '2026-07-16' },
      }),
      kenya,
    );
