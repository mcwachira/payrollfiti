...
export function listLeaveTypes(): Promise<LeaveType[]> {
  return apiFetch<LeaveType[]>('/v1/leave/types');
}
...
export function listLeaveRequests(filters?: {
  ...
  return apiFetch<LeaveRequest[]>(`/v1/leave/requests${query ? `?${query}` : ''}`);
}

export function listMyLeaveRequests(): Promise<LeaveRequest[]> {
  return apiFetch<LeaveRequest[]>('/v1/leave/requests/mine');
}
...
export function decideLeaveRequest(
  ...
  return apiFetch<LeaveRequest>(`/v1/leave/requests/${id}/decision`, {
    method: 'PATCH',
...
export function listLeaveBalances(
  ...
  return apiFetch<LeaveBalance[]>(
    `/v1/employees/${employeeId}/leave-balances${query}`,
  );
}
