...
export function listLoans(filters?: {
  ...
  return apiFetch<Loan[]>(`/v1/loans${query ? `?${query}` : ''}`);
}

export function listMyLoans(): Promise<Loan[]> {
  return apiFetch<Loan[]>('/v1/loans/mine');
}

export function getLoan(id: string): Promise<Loan> {
  return apiFetch<Loan>(`/v1/loans/${id}`);
}
...
export function decideLoan(
  ...
  return apiFetch<Loan>(`/v1/loans/${id}/decision`, {
    method: 'PATCH',
...
export function payoffLoan(id: string, note?: string): Promise<Loan> {
  return apiFetch<Loan>(`/v1/loans/${id}/payoff`, {
    method: 'PATCH',
